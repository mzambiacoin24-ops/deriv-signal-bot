...
class SymbolMemory:
    """Persistent memory of symbol/feed behaviour and signal outcomes."""
    ...
    def _save(self):
        ...
        fd, temp_path = tempfile.mkstemp(
            prefix="symbol_memory_", suffix=".tmp", dir=directory, text=True
        )
        try:
        ...
        if key not in symbols or not isinstance(symbols[key], dict):
            symbols[key] = {
                ...
                "stats": {
                    ...
                    "pending": 0, "buy": 0, "sell": 0, "tp_rate": 0.0,
                },
                "patterns": {}, "events": [], "updated_at": self._now(),
            }
        item = symbols[key]
    ...
    def record_signal(self, symbol, feed_label, display_name, signal):
        ...
        event_id = f"{symbol}:{int(signal.get('entry_epoch', 0))}:{len(item['events']) + 1}"
        event = {
            "id": event_id, "type": "signal", "created_at": self._now(),
            "status": "pending", "direction": direction,
            "entry": signal.get("entry"), "tp": signal.get("tp"),
            "sl": signal.get("sl"), "rr": signal.get("rr"),
            "quality": signal.get("quality"), "confidence": signal.get("confidence"),
            "setup": signal.get("setup"), "sweep": signal.get("sweep"),
            "regime": signal.get("regime"), "location": signal.get("location"),
            "d1": signal.get("d1"), "h4": signal.get("h4"),
            "m30": signal.get("m30"), "m15": signal.get("m15"), "m5": signal.get("m5"),
            "m1": signal.get("m1"),
            "movement_direction": signal.get("movement_direction"),
            "volatility": signal.get("volatility"),
            "movement_state": signal.get("movement_state"),
            "movement_score": signal.get("movement_score"),
            "movement_pressure": signal.get("movement_pressure"),
            "rsi": signal.get("rsi"), "sma": signal.get("sma"),
            "entry_epoch": signal.get("entry_epoch"), "result": None,
        }
    ...
    def record_result(self, symbol, event_id, result, exit_price=None, exit_epoch=None):
        ...
        if normalized not in {"tp", "sl", "cancelled", "expired"}:
            return False
        ...
        stats = item["stats"]
        stats["pending"] = max(0, int(stats.get("pending", 0)) - 1)
        if normalized == "tp":
            stats["wins"] = int(stats.get("wins", 0)) + 1
        elif normalized == "sl":
            stats["losses"] = int(stats.get("losses", 0)) + 1
        closed = int(stats.get("wins", 0)) + int(stats.get("losses", 0))
        stats["tp_rate"] = round((int(stats.get("wins", 0)) / closed) * 100, 2) if closed else 0.0
    ...
    def _learn_pattern(self, item, event):
        key = "|".join([
            str(event.get("direction", "unknown")),
            str(event.get("setup", "unknown")),
            str(event.get("sweep", "none") or "none"),
            str(event.get("regime", "unknown")),
            str(event.get("volatility", "unknown")),
            str(event.get("movement_state", "unknown")),
        ])
        pattern = item["patterns"].setdefault(key, {
            "signals": 0, "wins": 0, "losses": 0, "tp_rate": 0.0,
        })
        pattern["signals"] += 1
        ...
        pattern["tp_rate"] = round((pattern["wins"] / closed) * 100, 2) if closed else 0.0
    ...
    def get_stats(self, symbol):
        ...
        return None if item is None else dict(item.get("stats", {}))

    def get_patterns(self, symbol):
        item = self.get_symbol(symbol)
        return {} if item is None else dict(item.get("patterns", {}))
