...
class PublicMarketClient:
    ...
    def _request_sync(self, payload):
        ws = None
        try:
            ws = websocket.create_connection(PUBLIC_WS_URL, timeout=self.timeout)
    ...
    async def get_ohlc(self, symbol, timeframe="1m", count=200):
        ...
        return await self.get_candle_history(symbol, timeframe_map[timeframe], count)
    ...
    def _stream_worker(self, symbol, granularity):
        ws = None
        current_candle = None
        try:
            def on_open(sock):
                sock.send(json.dumps({"ticks": symbol, "subscribe": 1}))
                print(f"[{symbol}] Tick stream connected")
            ...
            def on_message(sock, message):
                ...
                try:
                    ...
                    if "error" in response:
                        error = response["error"]
                        print(
                            f"Deriv stream error for {symbol}: "
                    ...
                    is_new_candle = (
                        ...
                        or current_candle["epoch"] != candle_epoch
                    )
                    ...
                    asyncio.run_coroutine_threadsafe(
                        callback(symbol, candle_copy), loop
                    )
                except Exception as exc:
                    print(f"Candle message error for {symbol}: {exc}")

            def on_error(sock, error):
                print(f"Deriv WebSocket error for {symbol}: {error}")

            def on_close(sock, status_code, message):
                print(
                    f"Deriv WebSocket closed for {symbol}: "
            ...
            while not self._closed:
                try:
                    ...
                    if self._closed:
                        break
                    print(f"Stream disconnected for {symbol}: {exc}")
                if self._closed:
                    break
                time.sleep(2)
        finally:
