...
class MovementEngine:
    ...
    Tracks pressure, expansion, rejection, acceleration and failed
    breakouts. A failed breakout can therefore produce a reversal
    direction before a conventional fractal structure flip.
    """
    ...
    def _levels(self):
        ...
        if len(candles) < 6:
            return None, None
        previous = candles[-21:-1] or candles[:-1]
        if not previous:
            return None, None
        return (
    ...
    def _detect_liquidity_event(self):
        ...
        c = candles[-1]
        high_level, low_level = self._levels()
        avg = self._avg_range(20)
        tolerance = avg * 0.08 if avg else 0.0
        ...
        if (
            ...
            and c.get("epoch") != self.breakout_epoch
            and c["close"] < self.breakout_level
        ):
            event = "FAILED_BREAKOUT_HIGH"
            level = self.breakout_level
            self.reversal_direction = "down"
            self.reversal_epoch = c.get("epoch")
            ...
            and c["high"] > high_level + tolerance
            and c["close"] < high_level
        ):
            event = "SWEEP_HIGH"
            level = high_level
            self.reversal_direction = "down"
            ...
            and c["low"] < low_level - tolerance
            and c["close"] > low_level
        ):
            event = "SWEEP_LOW"
            level = low_level
            self.reversal_direction = "up"
        ...
        # Record a fresh breakout so the following rejection can be recognized.
        if (
            high_level is not None
            and c["high"] > high_level + tolerance
            and c["close"] >= high_level
            ...
            self.breakout_epoch = c.get("epoch")
            self.breakout_age = 0
        ...
        elif self.reversal_direction is not None and self.reversal_epoch is not None:
            age = sum(
                1
                for item in candles
                if item.get("epoch") is not None
                and item.get("epoch") >= self.reversal_epoch
            )
            if age > 6:
                self.reversal_direction = None
                self.reversal_epoch = None
    ...
    def snapshot(self, candle=None):
        ...
        raw_direction = (
            "up" if pressure >= 0.25
            else "down" if pressure <= -0.25
            else None
        )

        # Reversal direction has priority over raw candle pressure. This is
        # what allows a failed breakout to change the advisory direction.
        direction = self.reversal_direction or raw_direction
        momentum = pressure

        if direction == "down" and self.reversal_direction == "down":
            momentum = min(momentum, -0.25)
        elif direction == "up" and self.reversal_direction == "up":
            momentum = max(momentum, 0.25)
