...
class SMCAnalyzer:
    """
    Volatility-index price-action analyzer.

    This is designed for the three Deriv feeds only. It does not treat
    forex/crypto assumptions as the primary model.
    ...
    Core idea:
        ...
        -> optional FVG/retrace
        -> entry confirmation

    Important:
    - A previous high/low is a reaction AREA, not a guaranteed reversal.
    - A sweep alone never creates a signal.
    - FVG is supporting confluence, not mandatory.
    - The signal_bot decides final 1D/15M/5M/1M alignment.
    """
    ...
    def add_candle(self, candle):
        ...
        # Replace an unfinished candle rather than ageing the
        # narrative twice.
        if (
            self.candles
            and c.get("epoch") is not None
    ...
    def _liquidity_tolerance(self):
        ...
        # Previous highs/lows on Volatility indices can be
        # touched/overshot slightly before reacting.
        return (
            avg * 0.08
            if avg > 0
            else 0.0
    ...
    def _detect_liquidity_event(
        ...
        if (
            ...
            == "up"
            and self.breakout_level
            is not None
            and candle.get("epoch")
            != self.last_breakout_epoch
            and candle["close"]
            < self.breakout_level
        ):
            sweep = "high"
        ...
        # Clean breakout is recorded, but not immediately treated
        # as a reversal.
        if (
            candle["high"]
            > high_level + tolerance
        ...
        # Don't replace an already valid narrative with another
        # sweep until the current narrative expires.
        if self.pending_direction in (
            "up",
            "down",
        ):
            return
        ...
        self.pending_sweep_candle = (
            dict(candle)
        )

        self.pending_mss_level = (
            self._find_mss_level(
                self.pending_direction,
                candle,
            )
        )

        self.pending_target_liquidity = (
            self._find_target_liquidity(
                self.pending_direction,
                float(candle["close"]),
            )
        )
    ...
    def _displacement_ok(
        ...
        # Require meaningful movement, but do not require a
        # giant candle. This is important for Volatility 50/100.
        if (
            avg > 0
            and rng < avg * 0.95
        ):
        ...
        # Avoid chasing extreme spike candles.
        if (
            avg > 0
            and rng > avg * 2.80
        ):
    ...
    def _retracement_into_fvg(
        ...
        return (
            float(candle["low"])
            <= float(
                self.pending_fvg["high"]
            )
            and float(candle["high"])
            >= float(
                self.pending_fvg["low"]
            )
        )
    ...
    def _confirmation_ok(
        ...
        avg = self._average_range(
            20
        )

        # Do not enter after a giant chase candle.
        if (
            avg > 0
            and rng > avg * 2.40
        ):
    ...
    def _advance_narrative(
        ...
        if (
            ...
            if self._displacement_ok(
                ...
                self.pending_displacement_epoch = (
                    candle.get("epoch")
                )

                self.last_event = (
                    "DISPLACEMENT_AFTER_LIQUIDITY"
                )
        ...
        if (
            ...
            if self._mss_confirmed(
                ...
                self.last_event = (
                    "MSS_AFTER_DISPLACEMENT"
                )

                fvg = (
                    self._detect_fvg_at_end(
                        direction
                    )
                )
        ...
        if (
            ...
            if self._retracement_into_fvg(
                ...
                self.pending_retracement_epoch = (
                    candle.get("epoch")
                )
        ...
        if (
            ...
            target = (
                ...
                or self.pending_target_liquidity
            )
    ...
    def get_levels(self):
        ...
        return {
            ...
            "recent_liquidity_high": (
                recent_high
            ),
            "recent_liquidity_low": (
                recent_low
            ),
