...
class SMCAnalyzer:
    ...
    def _detect_sweep(self, candle):
        ...
        swept_high = candle["high"] > previous_high and candle["close"] < previous_high
        swept_low = candle["low"] < previous_low and candle["close"] > previous_low
    ...
    def _detect_choch(self):
        ...
        current = candles[-1]
        window = candles[-7:-1]
        previous_high = max(c["high"] for c in window)
        previous_low = min(c["low"] for c in window)
    ...
    def add_candle(self, candle):
        ...
        # Deriv sends many ticks while one candle is still open. Replace the
        # current candle instead of appending another candle for every tick.
        ...
        # A live signal must have a candle identity. Without epoch there is
        # no safe way to guarantee one signal per candle.
        if epoch is None:
            return None
        ...
        ob = self._find_order_block(choch)
        if ob is None:
            return None

        # Hard SMC-level duplicate protection.
