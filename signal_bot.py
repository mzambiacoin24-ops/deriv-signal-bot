...
MIN_SECONDS_BETWEEN_SIGNALS = int(
    os.getenv("MIN_SECONDS_BETWEEN_SIGNALS", "1800")
)
...
class PairMonitor:
    ...
    async def _maybe_send_signal(self, entry, price):
        ...
        if (
            ...
            log.info(
                "[%s] Imekataliwa: haiafiki HTF(15m) bias (%s).",
                self.display_name,
                self.htf.trend,
            )
            return
        ...
        if fvg is None:
            log.info(
                "[%s] Imekataliwa: hakuna Fair Value Gap kwenye msukumo huu.",
                self.display_name,
            )
            return
        ...
        if direction == "up" and (
            ...
            log.info(
                "[%s] Imekataliwa na RSI/SMA filter (UP).",
                self.display_name,
            )
            return
        ...
        if direction == "down" and (
            ...
            log.info(
                "[%s] Imekataliwa na RSI/SMA filter (DOWN).",
                self.display_name,
            )
            return
        ...
        if (
            primary_swept is None
            or primary_swept == secondary_swept
        ):
            log.info(
                "[%s] Imekataliwa: SMT divergence haikuthibitika (primary=%s, secondary=%s).",
                self.display_name,
                primary_swept,
                secondary_swept,
            )
        ...
        if sl_price is None or tp_price is None:
            log.info(
                "[%s] Imekataliwa: hakuna support/resistance ya kutosha kuamua SL/TP salama.",
                self.display_name,
            )
            return
        ...
        if rr < MIN_RR_RATIO:
            log.info(
                "[%s] Imekataliwa: R:R (%.2f) iko chini ya kikomo cha %.2f.",
                self.display_name,
                rr,
                MIN_RR_RATIO,
            )
        ...
        await self.telegram.send(
            ...
            f"⚖️ R:R = 1:{rr:.2f}\n"
            f"{lot_line}"
            f"Muundo: HTF(15m) bias={self.htf.trend.upper()} + LTF(1m) CHoCH+FVG+OB retest\n"
        ...
        log.info(
            "[%s] ISHARA %s imetumwa. Entry: %.4f | TP: %.4f | SL: %.4f | R:R=1:%.2f",
            self.display_name,
            direction.upper(),
            price,
            tp_price,
...
async def main():
    ...
    names = ", ".join(
        p[2] for p in SYMBOL_PAIRS
    )

    await telegram.send(
        f"🤖 <b>Signal Bot v3 imeanza (SMC/SMT + HTF/LTF + RSI/SMA + S/R-based SL/TP)</b>\n"
        f"Symbols: {names}\n"
        f"HTF bias: {HTF_GRANULARITY}s | LTF entry: {LTF_GRANULARITY}s\n"
        f"Vigezo: HTF match + FVG + RSI/SMA + SMT divergence (SHARTI) + "
...
if __name__ == "__main__":
    ...
    while True:
        ...
        try:
            ...
            log.error(
                "Bot imeanguka: %s. Inaanza upya baada ya %ds...",
                e,
                backoff,
            )
